...
namespace Sipro.SourceControlProvider.SiproSCP
{
    ...
    [MsVsShell.ProvideOptionPageAttribute(typeof(SiproSCPOptions), "Sipro Source Control Provider", "Sample Options Page", 106, 107, false)]
    [ProvideToolsOptionsPageVisibility("Sipro Source Control Provider", "Sample Options Page", "B0BAC05D-0000-41D1-A6C3-704E6C1A3DE2")]
    // Register a sample tool window visible only when the provider is active
    [MsVsShell.ProvideToolWindow(typeof(SiproSCPToolWindow))]
    [MsVsShell.ProvideToolWindowVisibility(typeof(SiproSCPToolWindow), "B0BAC05D-0000-41D1-A6C3-704E6C1A3DE2")]
    // Register the source control provider's service (implementing IVsScciProvider interface)
    [MsVsShell.ProvideService(typeof(SiproSCPService), ServiceName = "Sipro Source Control Provider Service")]
    // Register the source control provider to be visible in Tools/Options/SourceControl/Plugin dropdown selector
    [ProvideSourceControlProvider("Managed Sipro Source Control Provider", "#100")]
    ...
    public sealed class SiproSCP : MsVsShell.Package, 
        IOleCommandTarget,
        IVsPersistSolutionProps    // We'll write properties in the solution file to track when solution is controlled; the interface needs to be implemented by the package object
    {
        ...
        private SiproSCPService sccService = null;
        ...
        private const string _strProviderName = "Sipro Source Control Provider:{B0BAC05D-2000-41D1-A6C3-704E6C1A3DE2}";
        // The name of the solution section used to persist provider options (should be unique)
        private const string _strSolutionPersistanceKey = "SiproSourceControlProviderrSolutionProperties";
        ...
        private const string _strSolutionUserOptionsKey = "SiproSourceControlProvider";
        ...
        /////////////////////////////////////////////////////////////////////////////
        // SiproSCP Package Implementation
        ...
        protected override void Initialize()
        {
            ...
            sccService = new SiproSCPService(this);
            ((IServiceContainer)this).AddService(typeof(SiproSCPService), sccService, true);
            ...
            {
                // ToolWindow Command
                CommandID cmd = new CommandID(GuidList.guidSiproSCPCmdSet, CommandId.icmdViewToolWindow);
                MenuCommand menuCmd = new MenuCommand(new EventHandler(Exec_icmdViewToolWindow), cmd);
                mcs.AddCommand(menuCmd);

                // ToolWindow's ToolBar Command
                cmd = new CommandID(GuidList.guidSiproSCPCmdSet, CommandId.icmdToolWindowToolbarCommand);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdToolWindowToolbarCommand), cmd);
                mcs.AddCommand(menuCmd);

                // Source control menu commmads
                cmd = new CommandID(GuidList.guidSiproSCPCmdSet, CommandId.icmdAddToSourceControl);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdAddToSourceControl), cmd);
                mcs.AddCommand(menuCmd);

                cmd = new CommandID(GuidList.guidSiproSCPCmdSet, CommandId.icmdCheckin);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdCheckin), cmd);
                mcs.AddCommand(menuCmd);

                cmd = new CommandID(GuidList.guidSiproSCPCmdSet, CommandId.icmdCheckout);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdCheckout), cmd);
                mcs.AddCommand(menuCmd);

                cmd = new CommandID(GuidList.guidSiproSCPCmdSet, CommandId.icmdHistory);
                menuCmd = new MenuCommand(new EventHandler(Exec_icmdHistory), cmd);
                mcs.AddCommand(menuCmd);

                cmd = new CommandID(GuidList.guidSiproSCPCmdSet, CommandId.icmdUseSccOffline);
            ...
            rscp.RegisterSourceControlProvider(GuidList.guidSiproSCP);
        }
        ...
        // This function is called by the IVsSiproSCP service implementation when the active state of the provider changes
        ...
        public int QueryStatus(ref Guid guidCmdGroup, uint cCmds, OLECMD[] prgCmds, System.IntPtr pCmdText)
        {
            ...
            if (guidCmdGroup != GuidList.guidSiproSCPCmdSet)
            {
        ...
        OLECMDF QueryStatus_icmdHistory()
        {
            if (!IsThereASolution())
            {
        ...
        private void Exec_icmdHistory(object sender, EventArgs e)
        {
            ...
            foreach (string file in files)
            {
                ViewHistoryFile h = new ViewHistoryFile();
                h.file = file;
                h.Show();
            }
        ...
        {
            MsVsShell.ToolWindowPane window = this.FindToolWindow(typeof(SiproSCPToolWindow), 0, true);
        ...
        {
            SiproSCPToolWindow window = (SiproSCPToolWindow)this.FindToolWindow(typeof(SiproSCPToolWindow), 0, true);
