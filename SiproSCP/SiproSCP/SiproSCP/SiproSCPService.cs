...
// SiproSCPService.cs : Implementation of Sample Source Control Provider Service
//
...
namespace Sipro.SourceControlProvider.SiproSCP
{
        ...
        IDisposable 
    {
        ...
        private SiproSCP _SiproSCP = null;
        ...
        #region SiproSCP Service initialization/unitialization

        public SiproSCPService(SiproSCP SiproSCP)
        {
            Debug.Assert(null != SiproSCP);
            _SiproSCP = SiproSCP;

            // Subscribe to solution events
            IVsSolution sol = (IVsSolution)_SiproSCP.GetService(typeof(SVsSolution));
            ...
            IVsTrackProjectDocuments2 tpdService = (IVsTrackProjectDocuments2)_SiproSCP.GetService(typeof(SVsTrackProjectDocuments));
        ...
        //--------------------------------------------------------------------------------
        // IVsSiproSCP specific functions
        //--------------------------------------------------------------------------------
        #region IVsSiproSCP interface functions
        ...
        {
            if (pszProvider.CompareTo(_SiproSCP.ProviderName)!=0)
            {
            ...
            if (pscp2Project == null)
            {
                ...
                Trace.WriteLine(String.Format(CultureInfo.CurrentUICulture, "Solution {0} is registering with source control - {1}, {2}, {3}, {4}", _SiproSCP.GetSolutionFileName(), pszSccProjectName, pszSccAuxPath, pszSccLocalPath, pszProvider));

                IVsHierarchy solHier = (IVsHierarchy)_SiproSCP.GetService(typeof(SVsSolution));
                string solutionFile = _SiproSCP.GetSolutionFileName();
                SiproSCPStorage storage = new SiproSCPStorage(solutionFile);
                _controlledProjects[solHier] = storage;
            }
            else
            {
                Trace.WriteLine(String.Format(CultureInfo.CurrentUICulture, "Project {0} is registering with source control - {1}, {2}, {3}, {4}", _SiproSCP.GetProjectFileName(pscp2Project), pszSccProjectName, pszSccAuxPath, pszSccLocalPath, pszProvider));

                // Add the project to the list of controlled projects
                IVsHierarchy hierProject = (IVsHierarchy)pscp2Project;
                SiproSCPStorage storage = new SiproSCPStorage(_SiproSCP.GetProjectFileName(pscp2Project));
        ...
        public int UnregisterSccProject([InAttribute] IVsSccProject2 pscp2Project)
        {
            ...
            if (pscp2Project == null)
            {
                ...
                Trace.WriteLine(String.Format(CultureInfo.CurrentUICulture, "Solution {0} is unregistering with source control.", _SiproSCP.GetSolutionFileName()));
                hierProject = (IVsHierarchy)_SiproSCP.GetService(typeof(SVsSolution));
            }
            else
            {
                Trace.WriteLine(String.Format(CultureInfo.CurrentUICulture, "Project {0} is unregistering with source control.", _SiproSCP.GetProjectFileName(pscp2Project)));
        ...
        public int GetGlyphTipText([InAttribute] IVsHierarchy phierHierarchy, [InAttribute] uint itemidNode, out string pbstrTooltipText)
        {
            ...
            IList<string> files = _SiproSCP.GetNodeFiles(phierHierarchy, itemidNode);
            if (files.Count == 0)
            {
                return VSConstants.S_OK;
            }
        ...
        public int OnAfterCloseSolution([InAttribute] Object pUnkReserved)
        {
            ...
            _SiproSCP.SolutionHasDirtyProps = false;
        ...
        public int OnAfterOpenProject([InAttribute] IVsHierarchy pHierarchy, [InAttribute] int fAdded)
        {
            ...
            if (_SiproSCP.IsSolutionFolderProject(pHierarchy) && (fAdded == 1))
            {
                IVsHierarchy solHier = (IVsHierarchy)_SiproSCP.GetService(typeof(SVsSolution));
                if (IsProjectControlled(solHier))
                {
                    ...
                    RegisterSccProject(pSccProject, _solutionLocation, "", "", _SiproSCP.ProviderName);
                    ...
                    _SiproSCP.RefreshNodesGlyphs(nodes);
                }
            }
        ...
        public int OnAfterOpenSolution([InAttribute] Object pUnkReserved, [InAttribute] int fNewSolution)
        {
            ...
            if (_loadingControlledSolutionLocation.Length > 0)
            {
                ...
                RegisterSccProject(null, _loadingControlledSolutionLocation, "", "", _SiproSCP.ProviderName);
                ...
                Hashtable enumSolFolders = _SiproSCP.GetSolutionFoldersEnum();
                foreach (IVsHierarchy pHier in enumSolFolders.Keys)
                {
                    // Register this solution folder using the same location as the solution
                    IVsSccProject2 pSccProject = (IVsSccProject2)pHier;
                    RegisterSccProject(pSccProject, _loadingControlledSolutionLocation, "", "", _SiproSCP.ProviderName);
        ...
        public int QueryEditFiles([InAttribute] uint rgfQueryEdit, [InAttribute] int cFiles, [InAttribute] string[] rgpszMkDocuments, [InAttribute] uint[] rgrgf, [InAttribute] VSQEQS_FILE_ATTRIBUTE_DATA[] rgFileInfo, out uint pfEditVerdict, out uint prgfMoreInfo)
        {
            ...
            if (_SiproSCP.InCommandLineMode())
            {
                return VSConstants.S_OK;
            }
                                    ...
                                    if (fileExists && isFileReadOnly)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                IVsUIShell uiShell = (IVsUIShell)_SiproSCP.GetService(typeof(SVsUIShell));
        ...
        public int OnQueryRemoveFiles([InAttribute] IVsProject pProject, [InAttribute] int cFiles, [InAttribute] string[] rgpszMkDocuments, [InAttribute] VSQUERYREMOVEFILEFLAGS[] rgFlags, [OutAttribute] VSQUERYREMOVEFILERESULTS[] pSummaryResult, [OutAttribute] VSQUERYREMOVEFILERESULTS[] rgResults)
        {
            ...
            try
            {
                ...
                if (sccProject == null)
                {
                    ...
                    pHier = (IVsHierarchy)_SiproSCP.GetService(typeof(SVsSolution));
                    projectName = _SiproSCP.GetSolutionFileName();
                }
                else
                {
                    ...
                    if (sccProject != null)
                    {
                        projectName = _SiproSCP.GetProjectFileName(sccProject);
                    }
                }
                ...
                if (projectName != null)
                {
                    for (int iFile = 0; iFile < cFiles; iFile++)
                    {
                        SiproSCPStorage storage = _controlledProjects[pHier] as SiproSCPStorage;
                        if (storage != null)
                        {
        ...
        public void AddProjectsToSourceControl(ref Hashtable hashUncontrolledProjects, bool addSolutionToSourceControl)
        {
            ...
            foreach (IVsHierarchy pHier in hashUncontrolledProjects.Keys)
            {
                ...
                sccProject2.SetSccLocation("<Project Location In Database>", "<Source Control Database>", "<Local Binding Root of Project>", _SiproSCP.ProviderName);
            ...
            if (addSolutionToSourceControl)
            {
                ...
                _SiproSCP.SolutionHasDirtyProps = true;
            }
            ...
            {
                IVsSccProject2 sccProject2 = (IVsSccProject2)pHier;
                IList<string> files = _SiproSCP.GetProjectFiles(sccProject2);
                SiproSCPStorage storage = new SiproSCPStorage(_SiproSCP.GetProjectFileName(sccProject2));
        ...
        {
            foreach (SiproSCPStorage storage in _controlledProjects.Values)
            {
                if (storage != null)
                {
        ...
        public void AddFileToSourceControl(string file)
        {
            ...
            {
                SiproSCPStorage storage = _controlledProjects[vsItem.pHier] as SiproSCPStorage;
                if (storage != null)
                {
        ...
        public IList<VSITEMSELECTION> GetControlledProjectsContainingFile(string file)
        {
            ...
            foreach (IVsHierarchy pHier in _controlledProjects.Keys)
            {
                ...
                if (solHier == pHier)
                {
                    ...
                    if (file.ToLower().CompareTo(_SiproSCP.GetSolutionFileName().ToLower()) == 0)
                    {
                        VSITEMSELECTION vsItem;
                        vsItem.itemid = VSConstants.VSITEMID_ROOT;
                        vsItem.pHier = null;
