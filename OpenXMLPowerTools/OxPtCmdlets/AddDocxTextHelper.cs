...
namespace OpenXmlPowerTools
{
    ...
    {
        public static void AppendParagraphToDocument(
            string fileName,
            string strParagraph,
            bool isBold,
            bool isItalic,
            ...
            string styleName)
        {
            using (WordprocessingDocument wordprocessingDocument = WordprocessingDocument.Open(fileName, true))
            {
                ...
                if (!string.IsNullOrEmpty(foreColor))
                {
                    ...
                    if (colorValue == 0)
                        throw new AddDocxTextException(String.Format("The specified color {0} is unsupported, Please specify the valid color. Ex, Red, Green", foreColor));
...
AAsACwDBAgAAbCwAAAAA";
							...
							using (WordprocessingDocument doc = WordprocessingDocument.Open(memoryStream, true))
							{
								...
								Style templateStyle = doc.MainDocumentPart.StyleDefinitionsPart.Styles.Elements<Style>().Where(s => s.StyleId == styleName && s.Type == StyleValues.Paragraph).FirstOrDefault();

								//Check if the style is proper style. Ex, Heading1, Heading2
								if (templateStyle == null)
									throw new AddDocxTextException(String.Format("The specified style name {0} is unsupported, Please specify the valid style. Ex, Heading1, Heading2, Title", styleName));
								else
    ...
    public class AddDocxTextException : Exception
    {
        public AddDocxTextException(string message) : base(message) { }
    }
}
