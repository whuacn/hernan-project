...
function Send ()
{
	var result = generate(passwordform.secretpassword.value, passwordform.sitename1.value, passwordform.pswlength.value); 
...
function SetSiteName(tablink) {
	document.passwordform.sitename1.value = shortenurl(tablink);
}
document.addEventListener('DOMContentLoaded', function () {
	document.passwordform.secretpassword.focus();
	chrome.tabs.getSelected(null, function (tab) { SetSiteName(tab.url); });
	document.getElementById('buttonsubmit').addEventListener('click', Send);
});
*/
	...
	function generate(secretpassword, sitename, pswlength) {
	    var pwd = "";
        var error = "";
